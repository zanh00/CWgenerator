...
class WordCollectionService
{
    ...
    public static WordCollection? GetCollectionByName(string name, IEnumerable<WordCollection> collections)
    {
        if (collections == null || name == null || name.Length == 0)
            return null;
        else
            return collections.FirstOrDefault(c => string.Equals(c.Name, name, StringComparison.OrdinalIgnoreCase));
    }

    public static int GetCollectionIndexByName(string name, IEnumerable<WordCollection> collections)
    {
